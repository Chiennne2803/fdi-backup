...
import {AreaService} from "../../../../../../app/service/admin/area.service";
...
export class FuseVerticalNavigationCollapsableItemComponent implements OnInit, OnDestroy {
    ...
    toggleCollapsable(): void {
        if ((this.item.code && this.item.parentCode) && (!this.item.children || this.item.children.length <= 0)) {

            let codeC = undefined;
            if (this.item.parentCode == 'province') {
                codeC = 'district';
            } else  if (this.item.parentCode == 'district') {
                //luongnk comment, ko load commune
                // codeC = 'commune';
            }
                if (codeC) {
                    this._areaService.getAllByParentCategoriesCode(this.item.code).subscribe((res) => {
                        if (res) {
                            this.item.children = [];
                            res.payload.forEach((admCategoriesDTO: AdmCategoriesDTO) => {
                                if (codeC == 'commune') {
                                    this.item.children.push({
                                        title: admCategoriesDTO.categoriesName,
                                        type:  'basic' ,
                                        //luongnk comment, bo link khoi commune
                                        // link: `${ROUTER_CONST.config.admin.area.link}/` + codeC + "/" + admCategoriesDTO.categoriesCode,
                                        parentCode: codeC,
                                    ...
                                    this.item.children.push({
                                        title: admCategoriesDTO.categoriesName,
                                        // type: 'collapsable', //luongnk comment chuyen ve basic
                                        type: 'basic',
                                        link: `${ROUTER_CONST.config.admin.area.link}/` + codeC + "/" + admCategoriesDTO.categoriesCode,
                                        parentCode: codeC,
                                        parentId: admCategoriesDTO.admCategoriesId,
