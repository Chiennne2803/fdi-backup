...
export class DashboardComponent implements OnInit {
    ...
    chartCustomer: ApexOptions = {};
    chartTask: ApexOptions = {};
    chartProductTimeSeries: ApexOptions = {}
    ...
    ngOnInit(): void {
        this.chartOptions1 = {
            series: [
                {
                name: 'Số tiền tài khoản đầu tư mới',
                type: 'column',
                data: [25, 65, 93, 52, 70, 38]
                },
                {
                name: 'Số lượng tài khoản nhà đầu tư mới',
                type: 'line',
                data: [95, 65, 65, 78, 75, 45]
                }
            ],
            chart: {
                type: "line",
            ...
            markers: {
                size: 6, // dấu chấm tròn to hơn tại điểm line
                shape: 'circle'
            },
            dataLabels: {
                enabled: false,
            ...
            plotOptions: {
                bar: {
                columnWidth: '30%' // ❗ cột nhỏ lại
                }
            },
            colors: ["#1255FB", "#ED7632"],
            fill: {
                type: "gradient",
                gradient: {
                    shadeIntensity: 1,
        ...
        this._dashBoardService.dashboard$.subscribe((res) => {
            if (res) {
                ...
                if (res.tab3?.chartGrowthAccount) {
                    this.chartConfigAccount = this.chartGrowthAccount(res.tab3.chartGrowthAccount);
                }
                if (res.tab3?.chartGrowthSales) {
                    this.chartConfigSales = this.chartGrowthSales(res.tab3.chartGrowthSales);
                }
                if (res.tab3?.chartGrowthPay) {
                    this.chartConfigPay = this.chartGrowthPay(res.tab3.chartGrowthPay);
                }
            }
        });
    }
    ...
    private chartGrowthAccount(data): {} {
        ...
        const value2 = [];
        data.map((value) => {
            label.push(value.date ? this._datetimePipe.transform(new Date(value.date).getTime(), 'DD/MM/YYYY') : "");
    ...
    private initChart(): {
        plotOptions: { bar: { columnWidth: string } }; legend: { offsetX: number; offsetY: number; horizontalAlign: string; floating: boolean; position: string }; tooltip: { followCursor: boolean; theme: string }; title: { offsetX: number; margin: number; offsetY: number; floating: boolean; style: { fontFamily: undefined; color: string; fontSize: string; fontWeight: string }; text: string; align: string }; stroke: { width: number[] }; yaxis: { labels: { formatter: (value: number) => string; offsetX: number; minWidth: number; style: { colors: string } } }; colors: string[]; labels: any[]; states: { hover: { filter: { type: string; value: number } } }; dataLabels: { enabled: boolean }; xaxis: { axisTicks: { color: string }; tooltip: { enabled: boolean }; axisBorder: { show: boolean }; labels: { style: { colors: string } } }; series: any[]; grid: { borderColor: string; row: { opacity: number; colors: string[] } }; markers: { size: number }; chart: { dropShadow: { color: string; top: number; left: number; blur: number; opacity: number; enabled: boolean }; toolbar: { show: boolean }; zoom: { enabled: boolean }; type: string; height: number }
    } {
        let chartConfig = {
            title: {
                text: "X",
            ...
            yaxis: {
                labels: {
                    minWidth: 40,
                    formatter: (value: number): string => new CurrencyFormatPipe().transform(value, 3),
                    offsetX: -16,
                    style: {
                        colors: 'var(--fuse-text-secondary)'
                    }
