...
export class ContactInfoComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    initFormToEditing(title: string, contact?: AdmDeputyContactDTO): void {
        ...
        this.contactForm = this._fb.group({
            ...
            fullName: [contact?.fullName || '', [Validators.required, Validators.maxLength(50)]],
            referenContact: [contact?.referenContact || '', [Validators.required, Validators.maxLength(50)]],
            ...
            identification: [contact?.identification || '', [Validators.required, Validators.maxLength(12)]],
            idDate: [contact?.idDate ? new Date(contact.idDate) : null, Validators.required],
            idAddress: [contact?.idAddress || '', [Validators.required, Validators.maxLength(100)]],
            mobile: [contact?.mobile || '', [Validators.required, Validators.minLength(10), Validators.maxLength(11), forbiddenPhoneNumberValidator()]],
            email: [contact?.email || '', [Validators.required, Validators.email, Validators.maxLength(100)]],
            job: [contact?.job || '', Validators.required],
            jobAddress: [contact?.jobAddress || '', [Validators.maxLength(100)]],
            facebook: [contact?.facebook || '', [Validators.maxLength(100)]],
            address1: [contact?.address1 || '', Validators.required],
            address2: [contact?.address2 || '', Validators.required],
        });

        // Nếu thêm mới thì reset contactSelected
        if (!contact) {
