...
import {PersonalIncomeTaxService} from "../../../../../service/admin/managementTransactionFee";
...
import {
    ...
    TABLE_PERSONAL_INCOME_TAX_REQUEST
...
@Component({
    ...
    templateUrl: './personal-income-tax-request.component.html',
    encapsulation: ViewEncapsulation.None
})
export class PersonalIncomeTaxRequestComponent implements OnInit {
    @ViewChild('matDrawer', {static: true}) matDrawer: MatDrawer;
    public _dataSource$: Observable<BaseResponse>;
    public _taskbarConfig = TASK_BAR_CONFIG;
    public _tableConfig = TABLE_PERSONAL_INCOME_TAX_REQUEST;
                ...
                if (checkStatus) {
                    ...
                    confirmDialog.afterClosed().subscribe((res) => {
                        if ( res === 'confirmed' ) {
                            ...
                            this._personalIncomeTaxService.delete({ids: requestLock}).subscribe((res) => {
                                if (res.errorCode === '0') {
                                    this._alertService.showMessageSuccess('Xoá yêu cầu thành công');
                                    this._personalIncomeTaxService.searchPersonalIncomeTaxReq(this.searchPayload).subscribe();
                                } else {
        ...
        dialogRef.componentInstance.btnSearchClicked.subscribe(
            (response) => {
                if (response.action === 'reset') {
                    ...
                    this._personalIncomeTaxService.searchPersonalIncomeTaxReq({
                        ...response.form.value,
                        ...this.searchPayload,
    ...
    public handleRowClick(row: any): void {
        this._personalIncomeTaxService
            .getDetail({fsTranferWalletReqId: row.fsTranferWalletReqId})
            .subscribe((res) => {
                this._personalIncomeTaxService.openDetailDrawer();
            });
    }

    handleCloseDetailPanel(event: boolean): void {
        if (event) {
            this._personalIncomeTaxService.searchPersonalIncomeTaxReq().subscribe();
        }
        this._personalIncomeTaxService.closeDetailDrawer();
    ...
    checkboxItemChange(rows): void {
        ...
        this._tableBtnConfig = {
            commonBtn: [ {type: 'export', role: 'SFF_REQUEST_FUNDING_EXPORT', fileName : 'Thue_thu_nhap_Ca_nhan'},
                ...lstCommonBtn,
            ]
        };
    }
