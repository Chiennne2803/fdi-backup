...
import {AccountManagementFeeService, LoanArrangementFeeService} from 'app/service/admin/managementTransactionFee';
...
@Component({
    selector: 'detail-fee-loan-arrangement',
    templateUrl: './fee-loan-arrangement-detail.component.html',
    styleUrls: ['./fee-loan-arrangement-detail.component.scss'],
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations,
})
export class FeeLoanArrangementDetailComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit(): void {
        this.debtManagement = this._loanArrangementFeeService.selectedProfile$;
        ...
        this._loanArrangementFeeService.prepare().subscribe((res) => {
            if (res) {
    ...
    submit(): void {
        ...
        dialog.afterClosed().subscribe((result) => {
            if (result) {
                ...
                this._loanArrangementFeeService.doSignReq(request).subscribe((response) => {
                    if (response.errorCode === '0') {
                        this._fuseAlertService.showMessageSuccess(APP_TEXT.form.success.message);
                        this._loanArrangementFeeService.getDetail({fsTranferWalletReqId:this.requestId}).subscribe();
