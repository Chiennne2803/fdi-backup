...
@Component({
    ...
    templateUrl: './fee-investment-transaction-dialog.component.html',
    styleUrls: ['./fee-investment-transaction-dialog.component.scss'],
    providers: [DateTimeformatPipe]
})
export class FeeInvestmentTransactionDialogComponent implements OnInit {
    public requestCommissionForm: FormGroup = new FormGroup({});
    public _dataSource = new Observable<BaseResponse>();
    public _tableConfig = {
        columnDefinition: [
            new TextColumn('admAccountIdBeneficiaryName', 'Khách hàng', 10),
    ...
    constructor(
        ...
        private _matDialogRef: MatDialogRef<FeeInvestmentTransactionDialogComponent>,
    ...
    ngOnInit(): void {
        ...
        this._investmentTransactionFeeService.initTranferWalletReq(this.request).subscribe((res) => {
    ...
    submit(): void {
        ...
        dialog.afterClosed().subscribe((res) => {
            if (res === 'confirmed' && this.requestCommissionForm.valid) {
                ...
                this._investmentTransactionFeeService.create(request).subscribe((result) => {
                    if (result.errorCode === '0') {
                        this._matDialogRef.close(true);
                        this._investmentTransactionFeeService.searchInvestmentTransactionFee().subscribe();
