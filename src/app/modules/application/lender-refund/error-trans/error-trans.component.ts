...
import {TASK_BAR_CONFIG,TABLE_BUTTON_ERROR_TRANS_CONFIG, TABLE_ERROR_TRANS_CONFIG} from "./error-trans.config";


@Component({
    selector: 'app-error-trans',
    templateUrl: './error-trans.component.html'
})
export class ErrorTransComponent implements OnInit {
    @ViewChild('matDrawer', { static: true }) matDrawer: MatDrawer;
    dataSource$: Observable<BaseResponse>;
    detailError$: Observable<FsTranspayReqDTO>;

    public _taskbarConfig = TASK_BAR_CONFIG;
    public _tableConfig = TABLE_ERROR_TRANS_CONFIG;
    public _tableBtnConfig = TABLE_BUTTON_ERROR_TRANS_CONFIG;
    ...
    ngOnInit(): void {
        ...
        this.detailError$ = this._transPayReqService.transPayReqDetail$;
    ...
    reSubscribeData(): void {
        this._transPayReqService.doSearchErrorTransaction(this.searchPayload).subscribe();
    }
        ...
        dialogRef.componentInstance.btnSearchClicked.subscribe(
            (response) => {
                if (response.action === 'reset') {
                    this._transPayReqService.doSearchErrorTransaction({
                        ...this.searchPayload
                    }).subscribe();
                } else if (response.action === 'search') {
                    this._transPayReqService.doSearchErrorTransaction(response.form.value).subscribe();
                }
