...
import { CheckboxColumn, TextColumn, TransactionAmountColumn } from 'app/shared/models/datatable/display-column.model';
...
export class AccountStatementComponent implements OnInit {
    ...
    public _tableLoanConfig = {
        columnDefinition: [
            ...
            new TransactionAmountColumn('amount', 'Số tiền giao dịch(VND)', 15, false, 1),
            new TextColumn('newAmount', 'Số dư(VND)', 15, false, 1),
        ],
    ...
    public activeTab = 0;
    public searchPayloadBase: BaseRequest = new BaseRequest();
    public searchPayloadAuto: BaseRequest = new BaseRequest();
    ...
    public handleSearch($event: Event): void {
        ...
        if (this.activeTab === 0) {
            this.searchPayloadBase = {
                ...this.searchPayloadBase,
                quickSearch: keyword,
            };
        } else {
            this.searchPayloadAuto = {
                ...this.searchPayloadAuto,
                quickSearch: keyword,
            };
        }
    ...
    private doSearch(): void {
        const payloadBase = this.searchPayloadBase = { ...this.searchPayloadBase, isAuto: false };
        const payloadAuto = this.searchPayloadAuto = { ...this.searchPayloadAuto, isAuto: true };

        const payload =
            this.activeTab === 0 ? payloadBase : payloadAuto

        this._accountStatementService.doSearch(payload).subscribe({
            next: (res) => {
