...
import {DropListSearch, IDropList} from "../../../../shared/components/group-search/search-config.models";

@Component({
    selector: 'investor-rejected-profile',
    templateUrl: './rejected-profile.component.html',
    encapsulation: ViewEncapsulation.None
})
export class RejectedProfileComponent implements OnInit {
    @ViewChild('matDrawer', { static: true }) matDrawer: MatDrawer;
    public _dataSource$: Observable<BaseResponse>;
    public _taskbarConfig = TASK_BAR_CONFIG;
    public _tableConfig = { ...TABLE_INVESTOR_INVESTING_CONFIG, title: 'Hồ sơ đã từ chối' , isViewDetail: false};
        ...
        dialogRef.componentInstance.btnSearchClicked.subscribe(
            (response) => {
                if (response.action === 'reset') {
                    this._investorListService.reject().subscribe();
